...
import sre.task.Core.{Context, EdgeContext, VertexContext}
...
class TaskSpec extends FunSpec {
  ...
  object Helpers {
    def createIteratorForVertices(vertices: Iterator[IngraphVertex]) =
      new VertexIterator(vertices.map((v) => VertexContext(v, Map())))
    def createIteratorForEdges(edges: Iterator[IngraphEdge]) =
      new EdgeIterator(edges.map((e) => EdgeContext(e, Map())))
  }
  ...
  describe("VertexIterator") {
    describe("check") {
      ...
      val vertices = Helpers.createIteratorForVertices(indexer.vertexLookup.valuesIterator)

      it("should work for `name is not null or x.flavor = pistachio or null = null`") {

        IsNotNull(Null)
        IsNull(Null)
        Eq(This.Property("flavor"), Null)
        Eq(Null, Null)
//        val exp = Eq(
//          Or(
...
//        val actual = vertices.check(exp).map(_.head).to[Set]
//
//        assert(actual == Set(
...
//
//        val edges = Helpers.createIteratorForEdges(indexer.edgeLookup.valuesIterator)
//
